...
def main(test):
    ...
    #LCI = Laser Charge Injection
        ...
        #outputFolder = "\\\\EPLDT092\\LGAD_Project\\Oscilloscope_Data\\Gain_Sweep_TestsLHCB_PiN1_RTP_housing_OFF_RH_XX%_200V_03-11-22\\" 
        #outputFolder = "C:"

        ############################### IMPORTANT ###############################
        #If you don't change this, it will just overwrite your own work. 
        #No way to check this in code at the moment
        #outputFolder = outputFolder + "TCTBoard_PiN_L_FD1_35%_ND0.0_6.5VAmpBias_15Repeats_RTP_RH_XXper" #finishes %(12.4)15.6 (other2-0.8%)
        
        outputFolder = outputFolder + "TCTBoard_W3_F9_P_FD2_10%_ND0.0_6.5VAmpBias_30Repeats_PreIrrad_5.55%_RH_-17.35_degC"
        ...
        if ans.upper() == "Y":
            ...
            if deviceType == "LGAD":
                end1 = -450 #needs changing
                for i in range(0, end1, -25):
                    ps_Voltages.append(i)
                ps_Voltages.append(end1)
                
                #end2 = -480 # change
                #for i in range(-760, end2, -2): #change step size
                #    ps_Voltages.append(i)
                ...
                end2 = -200 # change
                for i in range(end, end2, -10): #change step size
                    ps_Voltages.append(i)
                ps_Voltages.append(end2)
            ...
            successStatus = 1;
            try:            
                successStatus=tests.jmRecordPulses(myOSC, outputFolder, ps, ps_Voltages,Imax,psInitialCurrentRange,full_step,ramp_down,kill_on_hit_compliance,rest_time,first_point_rest_time,repeats)
            except KeyboardInterrupt:
                print('Interrupted')
                try:
                    sys.exit(0)
    ...
    #Not sure this will 100% work
    if ps is not None:
        ps.controlSource('off')
        #ps.controlSource(1, 'off')
        #ps.controlSource(2, 'off')
        #ps.controlSource(3, 'off')
        ps.shutdown()
        #lcr.shutdown()
        #am.controlSource('off')
    #am.shutdown()
