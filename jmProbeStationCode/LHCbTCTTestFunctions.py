...
def arduino_temp_humid_setup():
    sens = serial.Serial("COM10", 9600)
    #time.sleep(5)
    #run first read during setup
    read_arduino_temp_humid(sens)
    return sens

    
def read_arduino_temp_humid(sensor):#,n_repeats=1):
    #readings = np.empty((n_repeats,2))  #stores calculated temp and humidty measurements
    #for i in range(n_repeats):
        #sensor.write(0x80)
        
        #time.sleep(0.15) #how long sensor waits
        
    #binary = sensor.readline()
    #binary = sensor.readlines()[-1]
    sensor.flushInput()
    binary = sensor.readline()
    string_n = binary.decode()
    print("IntTemp, ExtTemp, ExtRH: "+string_n)
...
def readArduino():
    with arduino_temp_humid_setup() as sens:
        out = read_arduino_temp_humid(sens)
...
if __name__ == '__main__':
    with arduino_temp_humid_setup() as ard:
        for i in range(10000):
            timeNow=time.time()
            time.sleep(1.25)
            
            readConditions = read_arduino_temp_humid(ard).rstrip()
...
def jmRecordPulses(myOSC, save_folder,ps,Voltages,Imax,InitCurrentRange,full_step,ramp_down,kill_on_hit_comp,rest_time,first_point_rest_time,repeats,measConditions):
    ...
    try:
        ...
        ##open file for writing 
            myOSC.mkdir(save_folder + '\\Conditions\\')
            
        time.sleep(2)
        
        if measConditions:
            conditionsFile = open(save_folder + '\\Conditions\\'+"conditionsData.csv","w")
            conditionsFile.write("Measurement, InternalTemp(C), ExternalTemp(C), ExternalRH, Meas. Voltage (V), Meas. Current (A)\n")
            
            #open arduino
            ard = arduino_temp_humid_setup()
        ...
        while index < len(Voltages) - 1:              
           ...
           #check current makes sense
            ...
            for i in range(repeats):
                ######
                # Trigger here the temp measurement
                ######
                ...
                if measConditions:
                    readConditions = read_arduino_temp_humid(ard).rstrip()
                    currVoltMeas, currCurrentMeas = ps.readVoltageAndCurrent()
                    conditionsFile.write(str(currVolt) + 'V_R' + str(currRepeat)+", "+readConditions+", "+str(currVoltMeas)+", "+str(currCurrentMeas)+"\n")
                ...
                if int(currRepeat)==0 or int(currRepeat)==int(repeats/2):
...
def mmChaosSearch(XlC, myOSC, linked_folder, save_folder, coords):
    """
    coords is a list of coordinates that need to be searched

    returns the coords and their respective integral value
    """
    #ill copy/paste a lot of jonathans stuff
    voltageTolerance=0.01 #V
    rampDownStep=5.0 #V
    rampDownWait=0.25 #s #Used to be 0.5, but the LabView used 0.1. Which is 5 times faster...
    nComplianceChecks=5
    ...
    try:
        ...
        overVoltageThreshold = 40e-3 #V marcus changed this to 100mV from 40mV, now back to 40mV. oops I was on 10V amp bias not 6.5V!
        
        print("settings zero position to here")
        XlC.set_zero_pos(XlC.deviceID_1)
        XlC.set_zero_pos(XlC.deviceID_2)

        #print(f"moving to first coordinate ({coords[0][0]}, {coords[0][1]}) from (0, 0) home which you have jmLaserAligned")
        XlC.move_to_microns(XlC.deviceID_1, coords[0][0])  #x                     
        XlC.wait_for_stop(XlC.deviceID_1, 100)
        XlC.move_to_microns(XlC.deviceID_2, coords[0][1])  #y                     
        XlC.wait_for_stop(XlC.deviceID_2, 100)

        for i in range(0, len(coords)):
            #skip the first coordinate as we shoul already be there
            #coords is a list of coordinated (x, y)
            print(f"moving to coordinate, ({coords[i][0]}, {coords[i][1]})")
            #how much the stage needs to move in x to get to new target (like a vector translation here)
            XlC.move_to_microns(XlC.deviceID_1, coords[i][0])                        
            XlC.wait_for_stop(XlC.deviceID_1, 100)
            #how much the stage needs to move in y to get to new target
            XlC.move_to_microns(XlC.deviceID_2, coords[i][1])                       
            XlC.wait_for_stop(XlC.deviceID_2, 100)
            #now we are at the correct coord, take the wave form and calc integral
                
            repeats_at_each_loc = 5
            
            avg_integral_list = []
            avg_height_list = []
             
            for i in range(0, repeats_at_each_loc):
                ...
                while True:           
                    ...
                    time.sleep(2) #Need to allow the OSC to perform this action                            
                ...
                #change these based on just how the waveform looks roughly
                #We could be more refined, but I think it's fine to be coarse (also see how this goes with tests)
                ...
                for j in range(len(Time)):
                    if Time[j] > startTime:
                        if Time[j] < endTime:
                            integral = integral + (timeStep * Volt[j])
                
                avg_integral_list.append(integral)
                avg_height_list.append(-min(Volt))
                
            #END FOR LOOP HERE
            
            #calculate average integral
            avg_integral = sum(avg_integral_list)/repeats_at_each_loc
            avg_height = sum(avg_height_list)/repeats_at_each_loc
            
            #having a list of just the output coords as well, just incase...
    ...
    print('Measurement finished at: {0}.'.format(time.strftime('%Y/%m/%d-%H:%M:%S')))
    return output_coords, integrals, heights
...
def jmFineSearch1D(XlC, myOSC, linked_folder, save_folder, scan_folder, scan_filename, device_ID):
    ...
    try:
        ...
        scanDistance = 400 #um  +- from home position #NN HERE
        #scanStep = 25
        scanStep = 20 #NN HERE
