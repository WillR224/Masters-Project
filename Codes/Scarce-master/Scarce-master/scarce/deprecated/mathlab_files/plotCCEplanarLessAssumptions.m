...
hold off;
title('Charge collection efficiencies for a planar silicon pixel sensor, 300 um electrode distance', 'FontWeight','bold','FontSize', 10);
xlabel('fluence [10^{15} N_{eq}/cm^2]', 'FontWeight','bold');
ylabel('CCE [%]', 'FontWeight','bold');
legend('less assumptions, see*','measured data','normal','Location', 'northeast');
% legend('3d sensor','planar sensor','Location', 'northeast');
grid on;
set(gca, 'GridLineStyle', '-');
ylim([0 100]);
set(gcf, 'Position', [100 100 100+600 100+350], 'Color', [1 1 1]);
export_fig('CCE_assumptions.pdf');
hold off;
